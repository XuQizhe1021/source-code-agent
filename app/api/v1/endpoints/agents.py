...
async def chat_with_agent(
    ...
    return await _build_chat_stream_response(
        ...
        is_share_access=is_share_access,
        token=token,
        api_key_id=None,
    )
...
async def _build_chat_stream_response(
    *,
    ...
    为什么抽这层：`chat_with_agent` 与 `chat_with_agent_api` 的核心编排一致，
    共享后可以保证修复和演进在同一位置完成，避免分叉逻辑长期漂移。
    """
    if not chat_request.stream:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="当前仅支持流式对话，请将 stream 设置为 true",
        )

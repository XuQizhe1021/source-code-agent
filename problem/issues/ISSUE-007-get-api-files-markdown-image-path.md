# ISSUE-007 GET /api/files/markdown-image/{path} 接口异常

## 1) 问题标题
GET /api/files/markdown-image/{path} 在覆盖测试中存在可复现异常
...
## 7) 证据引用（evidence 文件名）
- get-api-files-markdown-image-path.json
- server_startup.log
...
## 9) 初步定位（模块/函数/文件）
- 路由定义：文件管理 模块
- 接口函数：serve_markdown_image_api_files_markdown_image__path__get

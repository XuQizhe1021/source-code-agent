# ISSUE-006 GET /api/files/image/{file_id}/{image_name} 接口异常

## 1) 问题标题
GET /api/files/image/{file_id}/{image_name} 在覆盖测试中存在可复现异常
...
## 9) 初步定位（模块/函数/文件）
- 路由定义：文件管理 模块
- 接口函数：serve_image_api_files_image__file_id___image_name__get
